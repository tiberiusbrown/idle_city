...
Services seed placement uses the existing preview-and-confirm command path. It remains locked before Core, costs base 20 with the existing same-type exponential price, and contributes only matching Services influence within the fixed Manhattan radius of 8. A completed service building has a 4 x 4 footprint, capacity 8, an exterior entrance, a one-cell envelope, three staging cells, and a two-cell-wide ROW connector.
