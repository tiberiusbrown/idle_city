...
The headless balance runner reports active/allocated chunks, optional occupancy, demand, and ROW buffers, ROW/static-topology revisions, demand dirty/evaluation counts, path nodes expanded, path chunks touched, bounded traffic edge summaries and counters, incremental developer job progress, per-tick budget peaks, snapshot chunk summaries, invariant failures, and a determinism hash. Structural scenarios exercise sparse outward activation, coordinate extent beyond `1000 × 1000` without filling the box, localized demand dirtying, long multi-chunk routes, repeated traffic corridors, seed boundaries, same-type pricing, sparse local development, connected ROW, narrow connector budgets, dense circulation, supersession, and long incremental jobs.

## Deferred systems

This step intentionally does not add citizen construction labor, Services behavior, congestion costs, player-facing Traffic Analysis, paved roads, transit, chunk removal, ROW removal, player-facing expansion controls, polygon footprints, detailed voxel meshing, demolition, conversion, redevelopment, construction bots, a Travel Flow overlay, Traffic Analysis research state, or traffic-dependent routing. The next focused task is citizen-driven construction.
